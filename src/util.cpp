...
const char * const BITCOIN_CONF_FILENAME = "diverscoin.conf";
const char * const BITCOIN_PID_FILENAME = "diverscoin.pid";
...
#else
    const char* pszModule = "diverscoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Diverscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Diverscoin";
#else
    // Unix
    return pathRet / ".diverscoin";
#endif
#endif
}
