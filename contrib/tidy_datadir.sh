...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Diverscoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Diverscoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Diverscoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Diverscoin 0.7 datadir."
    ;;
  3)
    echo "Detected Diverscoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Diverscoin 0.8 datadir."
    ;;
esac
